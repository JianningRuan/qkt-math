  ...
  methods: {
    ...
    async getQuestions () {
      return new Promise(async (resolve) => {
        let res = await this.$api.test.getExercises()
        if (res && res.rcode === 300) {
    ...
    async finishChallenge () {
      ...
      let [err, res] = await awaitWrap(this.$api.test.submit(params))
      this.isFinish = true
      if (err) {
        return
      }
      if (res && res.rcode === 300) {
        let replace = res.data.replace
        this.$router.push({
          path: '/testResult',
          query: {
            replace
